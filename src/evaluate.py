...
# 过滤空 hypothesis 或 reference
filtered_pairs = [
    (h.strip(), r.strip()) for h, r in zip(hypotheses, references)
    if h.strip() and r.strip()
]

if not filtered_pairs:
    print("生成和参考描述均为空，跳过评估。")
    exit()

hypotheses, ref_texts = zip(*filtered_pairs)
hypotheses = list(hypotheses)
ref_texts = list(ref_texts)
references = [[r] for r in ref_texts]  # BLEU/METEOR 用 List[List[str]]
